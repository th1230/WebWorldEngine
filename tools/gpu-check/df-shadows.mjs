...
const { check, fail, finish } = startReport('距離場陰影：遠處也要有形狀');
...
try {
  ...
  console.log(
    `  暗掉的像素比例 開 ${(out.on.coverage * 100).toFixed(2)}%   關 ${(out.off.coverage * 100).toFixed(2)}%\n`,
  );
  ...
  const values = [
    ...
    out.other.shadow,
    out.other.open,
    out.other.terminator,
  ];
  check(
    values.every((v) => Number.isFinite(v)),
    `每個取樣點都在畫面上 —— ${values.map(f).join(', ')}`,
  );
  ...
  check(
    out.off.shadow > 0.9,
    `強度歸零之後影子也沒了 —— ${f(out.off.shadow)}（證明暗的是這個效果畫的）`,
  );
  check(
    errors.length === 0,
