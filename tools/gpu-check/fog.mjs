...
const { check, fail, finish } = startReport('體積霧：光柱要被擋住');
...
try {
  ...
  console.log(
    `  有距離場  牆後面 ${f(lum(out.withField.shadow))}   缺口 ${f(lum(out.withField.open))}`,
  );
  console.log(
    `  沒距離場  牆後面 ${f(lum(out.withoutField.shadow))}   缺口 ${f(lum(out.withoutField.open))}`,
  );
  console.log(
    `  透光率    影子裡 ${f(out.withField.shadow[3])}   影子外 ${f(out.withField.open[3])}\n`,
  );
  ...
  console.log(
    `  同一個像素開關場：牆後面 ${f(offShadow)} → ${f(onShadow)}（${shadowDrop.toFixed(1)}×），缺口 ${f(offOpen)} → ${f(onOpen)}（${openDrop.toFixed(1)}×）`,
  );
  ...
  check(onOpen < 0.28, `散射有隨著霧衰減 —— 缺口 ${f(onOpen)}（不衰減會到 0.37）`);
  check(
