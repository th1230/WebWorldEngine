...
try {
  for (const [label, orbit] of SPEEDS) {
    ...
    await page.waitForFunction(() => window.__ww?.totalFrames > 120, undefined, {
      timeout: 240000,
    });
    ...
    console.log(
      `  幀 p50 ${p50.toFixed(2)} ms   p95 ${p95.toFixed(2)} ms   最慢 ${sorted[sorted.length - 1].toFixed(2)} ms\n`,
    );

    console.log('  沒載入的幀 vs 有載入的幀');
    console.log(`    沒載入  ${mid(quiet, (s) => s.frameMs).toFixed(2)} ms（${quiet.length} 幀）`);
    console.log(
      `    有載入  ${mid(loading, (s) => s.frameMs).toFixed(2)} ms（${loading.length} 幀）`,
    );
    console.log(
      `    其中回呼自己 ${mid(loading, (s) => s.loadMs).toFixed(2)} ms，平均放 ${Math.round(mid(loading, (s) => s.placed)).toLocaleString()} 個\n`,
    );

    console.log(`  最慢的那 ${spikes.length} 幀（≥ p95）`);
    console.log(
      `    其中 ${withLoad.length} 幀正在載入（${((withLoad.length / Math.max(spikes.length, 1)) * 100).toFixed(0)}%）`,
    );
    console.log(
      `    平均整幀 ${mid(spikes, (s) => s.frameMs).toFixed(2)} ms，回呼自己 ${mid(spikes, (s) => s.loadMs).toFixed(2)} ms`,
    );
    ...
    console.log(
      `    **回呼佔尖峰的 ${share.toFixed(1)}%，其餘 ${(100 - share).toFixed(1)}% 在下游**\n`,
    );

    if (measured.budget !== null) {
      const b = measured.budget;
      console.log(
        `  自適應預算：基準 ${b.baselineMs.toFixed(2)} ms，預算 ${b.budgetMs.toFixed(2)} ms，載入速率 ${b.loadRate}`,
      );
    ...
    if (longTasks.length > 0) {
      ...
      console.log(
        `  長任務（>50 ms 的封鎖）：${longTasks.length} 次，共 ${total.toFixed(0)} ms，最長 ${worst.duration.toFixed(0)} ms`,
      );
    ...
    for (const [name, f] of PARTS) {
      ...
      console.log(
        `    ${name.padEnd(20)} 尖峰 ${hot.toFixed(2)} ms   安靜 ${cold.toFixed(2)} ms   差 ${delta >= 0 ? '+' : ''}${delta.toFixed(2)} ms`,
      );
    }
    if (worst !== null) {
      const explained =
        (worst.delta /
          Math.max(mid(spikes, (x) => x.frameMs) - mid(quiet, (x) => x.frameMs), 1e-6)) *
        100;
      console.log(`    **差最多的是「${worst.name}」，解釋了尖峰增量的 ${explained.toFixed(0)}%**`);
      if (explained < 40) {
        console.log(
          '    → 沒有一個分項解釋得了：尖峰在引擎量不到的地方（Three 自己的上傳／繪製）。',
        );
      }
    }
    console.log('');
    ...
    console.log(
      `  畫的東西：尖峰 ${Math.round(hotVis).toLocaleString()} 個可見 / ${Math.round(hotTri).toLocaleString()} 三角形`,
    );
    console.log(
      `            安靜 ${Math.round(coldVis).toLocaleString()} 個可見 / ${Math.round(coldTri).toLocaleString()} 三角形`,
    );
    ...
    if (perCell.length > 0) {
      ...
      console.log(
        `  每格的下游成本：p50 ${q(0.5).toFixed(2)} ms   p95 ${q(0.95).toFixed(2)} ms   最貴 ${perCell[perCell.length - 1].toFixed(2)} ms`,
      );
      ...
      console.log(`    每格 ${Math.round(
        mid(loading, (s) => s.placed) /
          Math.max(
            mid(loading, (s) => s.cells),
            1,
          ),
      )} 個 instance
`);
...
try {
  ...
  await page.goto(`${base}/?count=200000&spread=8000&orbit=200&rewrite=50&rewriteCount=400`, {
    waitUntil: 'load',
  });
  ...
  console.log(
    `    重寫幀   幀 ${mid(wrote, (x) => x.frameMs).toFixed(2)} ms   其中寫矩陣本身 ${mid(wrote, (x) => x.rewriteMs).toFixed(2)} ms`,
  );
  console.log(`    沒重寫   幀 ${mid(didnt, (x) => x.frameMs).toFixed(2)} ms`);
  console.log(
    `    三角形   重寫 ${Math.round(mid(wrote, (x) => x.triangles)).toLocaleString()}   沒重寫 ${Math.round(mid(didnt, (x) => x.triangles)).toLocaleString()}`,
  );
...
try {
  ...
  const page = await browser.newPage({ viewport: { width: 1280, height: 720 } });
  page.setDefaultNavigationTimeout(240000);
  await page.goto(`${base}/?count=200000&spread=8000&orbit=200&churn=50&churnCount=400`, {
    waitUntil: 'load',
  });
  await page.waitForFunction(() => window.__ww?.totalFrames > 120, undefined, { timeout: 240000 });
  ...
  console.log(
    `    走了   幀 ${mid(did, (x) => x.frameMs).toFixed(2)} ms   其中 writeMatrices 本身 ${mid(did, (x) => x.churnMs).toFixed(2)} ms`,
  );
  console.log(`    沒走   幀 ${mid(didnt, (x) => x.frameMs).toFixed(2)} ms`);
  console.log(
    `    三角形 走了 ${Math.round(mid(did, (x) => x.triangles)).toLocaleString()}   沒走 ${Math.round(mid(didnt, (x) => x.triangles)).toLocaleString()}`,
  );
  console.log(
    `    引擎 CPU 走了 ${mid(did, (x) => x.cpuMs).toFixed(2)} ms   沒走 ${mid(didnt, (x) => x.cpuMs).toFixed(2)} ms`,
  );
  console.log(
    `    HLOD 合併 走了 ${mid(did, (x) => x.hlodMerge).toFixed(2)} ms   沒走 ${mid(didnt, (x) => x.hlodMerge).toFixed(2)} ms`,
  );
  const delta = mid(did, (x) => x.frameMs) - mid(didnt, (x) => x.frameMs);
  const jsDelta = mid(did, (x) => x.churnMs);
  console.log(
    `    **多出來 ${delta.toFixed(2)} ms，其中 writeMatrices 的同步部分只佔 ${jsDelta.toFixed(2)} ms**`,
  );
  console.log(
    delta > 2
...
try {
  ...
  const page = await browser.newPage({ viewport: { width: 1280, height: 720 } });
  page.setDefaultNavigationTimeout(240000);
  await page.goto(`${base}/?count=200000&spread=8000&orbit=200&countToggle=50&countDelta=400`, {
    waitUntil: 'load',
  });
  await page.waitForFunction(() => window.__ww?.totalFrames > 120, undefined, { timeout: 240000 });
  ...
  console.log(
    `    改了   幀 ${mid(did, (x) => x.frameMs).toFixed(2)} ms   render() ${mid(did, (x) => x.renderMs).toFixed(2)} ms`,
  );
  console.log(
    `    沒改   幀 ${mid(didnt, (x) => x.frameMs).toFixed(2)} ms   render() ${mid(didnt, (x) => x.renderMs).toFixed(2)} ms`,
  );
  console.log(
    `    三角形 改了 ${Math.round(mid(did, (x) => x.triangles)).toLocaleString()}   沒改 ${Math.round(mid(didnt, (x) => x.triangles)).toLocaleString()}`,
  );
  console.log(
    `    引擎 CPU 改了 ${mid(did, (x) => x.cpuMs).toFixed(2)} ms   沒改 ${mid(didnt, (x) => x.cpuMs).toFixed(2)} ms`,
  );
  const delta = mid(did, (x) => x.frameMs) - mid(didnt, (x) => x.frameMs);
...
try {
  ...
  const run = async (query, label) => {
    ...
    console.log(`  ${label}`);
    console.log(
      `    幀 p50 ${q(0.5).toFixed(2)} ms   p95 ${q(0.95).toFixed(2)} ms   尖峰倍率 ${(q(0.95) / q(0.5)).toFixed(2)}×`,
    );
    console.log(
      `    有 cell 進來的 ${loading.length} 幀，合併的繪製 ${rows[rows.length - 1]?.merged ?? '—'}`,
    );
...
try {
  ...
  const run = async (query, label) => {
    ...
    console.log(`  ${label}`);
    console.log(
      `    幀 p50 ${q(0.5).toFixed(2)} ms   p95 ${q(0.95).toFixed(2)} ms   **尖峰倍率 ${(q(0.95) / q(0.5)).toFixed(2)}×**   有 cell 進來 ${loads} 幀`,
    );
