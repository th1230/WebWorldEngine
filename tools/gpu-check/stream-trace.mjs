...
try {
  ...
  console.log(
    `  主執行緒任務 ${tasks.length} 個：p50 ${q(0.5).toFixed(2)} ms，p95 ${q(0.95).toFixed(2)} ms，最長 ${durations[durations.length - 1].toFixed(2)} ms`,
  );
  ...
  const attribute = (task) => {
    const inside = complete.filter(
      (e) =>
        e.ts >= task.ts && e.ts + e.dur <= task.ts + task.dur && e !== task && e.name !== 'RunTask',
    );
    // 只取最上層的那些（不被其他被選中的事件包住）。
    const top = inside.filter(
      (e) => !inside.some((o) => o !== e && o.ts <= e.ts && o.ts + o.dur >= e.ts + e.dur),
    );
  ...
  for (const [name, ms] of ranked) {
    console.log(
      `    ${name.padEnd(34)} ${ms.toFixed(1)} ms（${((ms / heavyTotal) * 100).toFixed(0)}%）`,
    );
  }
  ...
  for (const f of frames) {
    ...
    const top = kids.filter(
      (e) => !kids.some((o) => o !== e && o.ts <= e.ts && o.ts + o.dur >= e.ts + e.dur),
    );
  ...
  for (const [name, ms] of innerRanked) {
    console.log(
      `    ${name.padEnd(34)} ${ms.toFixed(1)} ms（${((ms / frameTotal) * 100).toFixed(0)}%）`,
    );
  }
  const innerExplained = innerRanked.reduce((a, b) => a + b[1], 0);
  console.log(
    `    ${'（回呼裡沒有子事件的部分）'.padEnd(28)} ${(frameTotal - innerExplained).toFixed(1)} ms（${(((frameTotal - innerExplained) / frameTotal) * 100).toFixed(0)}%）`,
  );
