...
export const DEFAULT_RUNS: readonly SceneRun[] = [
  ...
  // ── 移動中串流 ───────────────────────────────────────────────────────
  //
  // 唯一一個內容在載入中變動的場景。其餘全部是一次擺完的靜態場景（包括
  // 1M 那次容量掃描），而靜態場景裡「格子重建多貴」是隱形的。
  //
  // 看的是 verdict 裡的 `spatial%`，不是幀時間本身 —— 幀時間會被載入的
  // 突發成本蓋掉，而退化的形態是「畫面正常、只有幀時間差」。
  //
  // 暖機拉長：串流要跑到常駐數穩定才算穩態，而那要走過好幾格。
  // 需要 pnpm cook:real。
  {
    id: 'streaming-move',
    warmup: 300,
