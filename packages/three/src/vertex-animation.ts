 ...
 *
 * 線性，大約 **0.27 µs / 頂點 / 幀**。所以自己的模型要多久是算得出來的：
 * 一個 30,000 頂點的角色烘 32 幀約 260 ms。
 *
 * ## 為什麼沒有搬去 cook 時做
 *
 * 因為在這個量級上不值得。真的變成問題（幾百毫秒的卡頓）時，正確的解法也
 * **不是 cook 而是 worker** —— 那條路已經有了（`autoLod` 就在 worker 裡跑），
 * 而且保住「不需要建置步驟」這件事。
 *
 * cook 時烘會更快，但那是加速不是前提 —— 與整個套件同一句話：用了更好，
 * 不用也能動。
 */
