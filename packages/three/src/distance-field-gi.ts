...
export interface DistanceFieldGiOptions extends DistanceFieldOptions {
   ...
   * ## 它**看不到材質**
   *
   * 表面快取讀的是幾何（頂點顏色）或這個選項，**不是** `mesh.material.color`
   * —— 材質是使用者的東西，而這裡拿到的只有一份 `BufferGeometry`。
   *
   * 所以這個值與材質的顏色是**同一個數字寫在兩個地方**，對不上的症狀是
   * 「反射與反彈的顏色跟看到的不一樣」。實測踩過：一個紅箱子的反射是白的。
   *
   * 與水那一節「一份波形兩邊共用」是同一類陷阱，差別在水可以共用同一條式子，
   * 而材質是使用者的物件 —— 只能靠這段文件擋。
   */
