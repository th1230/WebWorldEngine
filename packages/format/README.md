...
「cook 完載不進去」，訊息指向錯的方向。共用一份原始碼就沒有這個問題。

### 更正：npm 的版本解析**不是**那個守門員

這裡原本寫著「兩個工具解析到不同的 major，安裝當下就會被擋下來」。
**那句話是錯的**，而且是量出來的：
...
```text
資產 schema v1 與 runtime 的 v2 不符，請重新執行 pnpm cook
```

那個位置其實比安裝時更對：cook 出來的資產通常跟著網站部署，與使用者的
`node_modules` 是兩件事 —— 安裝時的檢查根本看不到那些檔案。
